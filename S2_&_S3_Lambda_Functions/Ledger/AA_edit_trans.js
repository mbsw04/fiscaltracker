...
export const handler = async (event) => {
  ...
  const { user_id, trans_id, credit, debit, credit_account_id, debit_account_id, description, comment, type, files } = body;
  ...
  try {
    ...
    if (files && files.zip_content) {
      try {
        ...
        const s3Key = `transactions/${trans_id}.zip`;
        
        // If replacing files, delete the old zip file first
        if (files.replace_existing) {
          try {
            ...
            console.log('Previous file not found, continuing with upload');
          }
        }
        ...
        const uploadParams = {
          ...
          Metadata: {
            'transaction-id': String(trans_id),
        ...
        await conn.execute(
          ...
          [
            ...
            JSON.stringify({
              ...
              replaced: files.replace_existing || false
            }),
            user_id
          ]
        );
        ...
        // Don't fail the transaction update if file upload fails
